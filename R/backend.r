...
detect_reference <- function(treatment, reference) {
    if (is.null(reference)) {
        ...
        assert(!(identical(unique(ref_), character(0))), "No 'reference' variable provided and non auto-detected.")
        assert(length(ref_) == 1, "Found more than one 'reference' variable in 'treatment' column.")
        return(ref_)
    }
    assert(any(reference %in% unique(treatment)), "Variable provided as 'reference' not present in input data.")
    return(reference)
}
